...
module.exports = vorpal => {
  ...
  vorpal
    ...
    .action(async (args, callback) => {
      ...
      if (args.things && args.things.indexOf("all") !== -1) {
  ...
  vorpal.command("init [path]", "build.....").action(async (args, callback) => {
    ...
    // console.log("copy over a sample dapp...", args);

    const sourceDappPath = path.join(__dirname, '../../.transmute-template/dapp') 
    const targetPath = path.join(process.cwd(), args.path) || process.cwd();
